...
const API_KEY = process.env.API_KEY || "54cfb51e6f2e98f3f0208cb91f6a1182";
const second_API_KEY =
  process.env.SECOND_API_KEY || "c90c3db7b6ea99ec7c9e6c6f21fe8a894f25e331";
const PORT = process.env.PORT || 3000;

// Initialize Google Gemini AI (Free tier: 15 req/min, 1,500 req/day)
// Get your free API key from: https://ai.google.dev/
const GEMINI_API_KEY =
  process.env.GEMINI_API_KEY || "AIzaSyDS7FaSpsQt1SUbF_T5MwcB44x99XFS38M";
...
function getPlantRecommendation(plantName) {
  return new Promise((resolve, reject) => {
    const results = [];
    const csvPath = path.join(__dirname, "Plants_Recommended_For_Planting.csv");
    fs.createReadStream(csvPath)
      ...
      .on("end", () => {
        ...
        console.log('Plant found in CSV:', plant.Crop);
        console.log('Raw economic values:');
        console.log('- Seed Cost raw:', JSON.stringify(plant["Seed Cost per Hectare (USD)"]));
        console.log('- Expected Yield raw:', JSON.stringify(plant["Expected Yield per Hectare (kg)"]));
        console.log('- Market Price raw:', JSON.stringify(plant["Market Price per kg (USD)"]));
        ...
        const plantData = {
          ...
          expectedYieldPerHectare: Number(
            plant["Expected Yield per Hectare (kg)"]
          ),
        ...
        console.log('Converted economic values:');
        console.log('- Seed Cost converted:', plantData.seedCostPerHectare);
        console.log('- Expected Yield converted:', plantData.expectedYieldPerHectare);
        console.log('- Market Price converted:', plantData.marketPricePerKg);
...
app.get("/api/agri-suitability-city", async (req, res) => {
  ...
  const tempSuitability =
    avgTemp >= plantProfile.minTemp && avgTemp <= plantProfile.maxTemp;
  const humiditySuitability =
    avgHumidity >= plantProfile.minHumidity &&
    avgHumidity <= plantProfile.maxHumidity;
  const aqiSuitability =
    avgAqi >= plantProfile.minAqi && avgAqi <= plantProfile.maxAqi;
  ...
  const pHSuitability =
    soilPH >= plantProfile.minSoilPH && soilPH <= plantProfile.maxSoilPH;

  // 8. SEASONAL ANALYSIS
  const currentMonth = new Date().getMonth() + 1;
  const seasonalSuitability = assessSeasonalSuitability(
    plantProfile,
  ...
  const irrigationPlan = generateIrrigationPlan(
    plantProfile,
    avgTemp,
    avgHumidity,
    estimatedFarmSize
  ...
  const totalCosts =
    Object.values(baseCosts).reduce((sum, cost) => sum + cost, 0) *
  ...
  const adjustedYield =
    baseYield *
    environmentalFactor *
    (soilSuitability ? 1.0 : 0.8) *
    (seasonalSuitability.factor || 0.9);
  ...
  res.json({
    ...
    environmental: {
      temperature: {
        ...
        value: avgTemp,
        optimal: [plantProfile.minTemp, plantProfile.maxTemp],
      },
      humidity: {
        suitable: humiditySuitability,
        value: avgHumidity,
        optimal: [plantProfile.minHumidity, plantProfile.maxHumidity],
      },
      airQuality: {
        suitable: aqiSuitability,
        value: avgAqi,
        optimal: [plantProfile.minAqi, plantProfile.maxAqi],
      },
    ...
    economics: {
      ...
      breakEven:
        totalCosts > 0
          ? Math.round(totalCosts / plantProfile.marketPricePerKg)
          : 0,
    },
  ...
  for (const city of cities) {
    try {
      ...
      const suitabilityResp = await fetch(
        `https://serv-virid.vercel.app/api/agri-suitability-city?city=${encodeURIComponent(
          city
        )}&plant=${encodeURIComponent(plant)}&soilType=${encodeURIComponent(
          soilType
        )}&soilPH=${soilPH}`
      );
...
const comprehensiveVegetableDatabase = {
  ...
  Spinach: {
    ...
    seedCostPerHectare: 180,
    expectedYieldPerHectare: 20000,
    averageMarketPrice: 3.0,
    profitMargin: 0.7,
  ...
  Carrot: {
    ...
    potassiumNeeds: "high",
    organicMatter: "10-15%",
    drainageNeeds: "excellent-drainage",

    seedCostPerHectare: 150,
    expectedYieldPerHectare: 35000,
    averageMarketPrice: 1.8,
    profitMargin: 0.6,
  ...
  Potato: {
    ...
    phosphorusNeeds: "high",
    potassiumNeeds: "very-high",
    organicMatter: "15-20%",
    drainageNeeds: "well-drained",

    seedCostPerHectare: 800,
    expectedYieldPerHectare: 45000,
    averageMarketPrice: 1.2,
    profitMargin: 0.55,
    ...
    waterRequirements: 500,
    irrigationFrequency: "deep watering weekly",
  ...
  Tomato: {
    ...
    seedCostPerHectare: 300,
    expectedYieldPerHectare: 60000,
    averageMarketPrice: 3.5,
    profitMargin: 0.75,
    ...
    waterRequirements: 600,
    irrigationFrequency: "consistent daily",
  ...
  Cucumber: {
    ...
    seedCostPerHectare: 250,
    expectedYieldPerHectare: 40000,
    averageMarketPrice: 2.0,
    profitMargin: 0.65,
  ...
  Pepper: {
    ...
    seedCostPerHectare: 400,
    expectedYieldPerHectare: 35000,
    averageMarketPrice: 4.0,
    profitMargin: 0.7,
    ...
    waterRequirements: 500,
    irrigationFrequency: "deep watering 2x/week",
  ...
  Eggplant: {
    ...
    seedCostPerHectare: 350,
    expectedYieldPerHectare: 30000,
    averageMarketPrice: 3.0,
    profitMargin: 0.65,
    ...
    waterRequirements: 550,
    irrigationFrequency: "consistent moisture",
...
app.post("/api/farmer-recommendation", async (req, res) => {
  ...
  try {
    ...
    const tempSuitability =
      currentTemp >= vegData.temp[0] && currentTemp <= vegData.temp[1];
    const humiditySuitability =
      currentHumidity >= vegData.humidity[0] &&
      currentHumidity <= vegData.humidity[1];

    // 4. Soil suitability analysis
    const pHSuitability =
      soilPH >= vegData.soilPH[0] && soilPH <= vegData.soilPH[1];
    ...
    const expectedRevenue =
      vegData.expectedYieldPerHectare * farmSize * vegData.averageMarketPrice;
    ...
    res.json({
      ...
      companionPlanting: {
        ...
        ecosystemBenefits:
          "Improved soil health, natural pest control, enhanced biodiversity",
      },
...
function generateModificationPlan(vegData, suitability) {
  ...
  if (!suitability.tempSuitability) {
    plan.push(
      `Adjust planting time - optimal temperature range is ${vegData.temp[0]}°C to ${vegData.temp[1]}°C`
    );
  }

  if (!suitability.pHSuitability) {
    plan.push(
      `Adjust soil pH to ${vegData.soilPH[0]}-${vegData.soilPH[1]} range using lime or sulfur`
    );
  }

  if (!suitability.soilTypeSuitability) {
    plan.push(
      `Improve soil structure - add organic matter for better drainage/retention`
    );
  }
  ...
  if (suitability.diseaseRisk !== "Low") {
    plan.push(
      `Implement preventive measures against ${vegData.commonDiseases.join(
        ", "
      )}`
    );
  }
...
app.post("/api/simulate-vegetable-farming", async (req, res) => {
  ...
  try {
    ...
    const plantProfile = {
      name: plant,
      // Environmental from CSV (primary source) - both array and individual properties
      tempRange: [csvPlantData.minTemp, csvPlantData.maxTemp],
      minTemp: csvPlantData.minTemp,
      maxTemp: csvPlantData.maxTemp,
      humidityRange: [csvPlantData.minHumidity, csvPlantData.maxHumidity],
      minHumidity: csvPlantData.minHumidity,
      maxHumidity: csvPlantData.maxHumidity,
      aqiRange: [csvPlantData.minAqi, csvPlantData.maxAqi],
      minAqi: csvPlantData.minAqi,
      maxAqi: csvPlantData.maxAqi,

      // Soil requirements from CSV - both array and individual properties
      soilPH: [csvPlantData.minSoilPH, csvPlantData.maxSoilPH],
      minSoilPH: csvPlantData.minSoilPH,
      maxSoilPH: csvPlantData.maxSoilPH,
      soilType: csvPlantData.soilTypes,
      soilTypes: csvPlantData.soilTypes,
    ...
    const tempSuitability =
      temp >= plantProfile.minTemp && temp <= plantProfile.maxTemp;
    const humiditySuitability =
      humidity >= plantProfile.minHumidity &&
      humidity <= plantProfile.maxHumidity;
    const aqiSuitability =
      aqi >= plantProfile.minAqi && aqi <= plantProfile.maxAqi;

    // 5. Soil suitability
    const pHSuitability =
      soilPH >= plantProfile.minSoilPH && soilPH <= plantProfile.maxSoilPH;
    const soilTypeSuitability =
      plantProfile.soilTypes && plantProfile.soilTypes.includes(soilType);
    ...
    console.log('=== ECONOMIC CALCULATIONS DEBUG ===');
    console.log('Plant profile economic values:');
    console.log('- seedCost:', plantProfile.seedCost);
    console.log('- expectedYield:', plantProfile.expectedYield);
    console.log('- marketPrice:', plantProfile.marketPrice);
    console.log('- farmSize:', farmSize);
    
    const seedCost = plantProfile.seedCost * farmSize;
    console.log('Initial seedCost calculation:', seedCost);

    // 8. Enhanced risk assessment with seasonal factors (BEFORE growth simulation)
    const diseaseRisk = getSeasonalDiseaseRisk(
      plantProfile,
      plantingMonth,
      temp,
      humidity
    ...
    const seasonalGrowthFactor = getSeasonalGrowthFactor(
      plantProfile,
      plantingMonth
    );
    ...
    if (overallGrowthFactor > 0.8)
      r = 0.12; // Excellent conditions = faster growth
    else if (overallGrowthFactor < 0.5)
      r = 0.04; // Poor conditions = much slower growth
    ...
    for (
      let t = 0;
      t <= plantProfile.daysToMaturity;
      t += Math.max(1, Math.floor(plantProfile.daysToMaturity / 30))
    ) {
    ...
    if (
      growthSeries[growthSeries.length - 1].day < plantProfile.daysToMaturity
    ) {
    ...
    console.log(`Final Scores for ${plant} in ${plantingMonth}:`, {
      ...
      finalBiomass:
        growthSeries[growthSeries.length - 1]?.biomass?.toFixed(1) || "N/A",
      finalHealth:
        growthSeries[growthSeries.length - 1]?.healthIndex?.toFixed(1) + "%" ||
        "N/A",
    ...
    let yieldAdjustmentFactor =
      baseYieldAdjustment * 0.6 + healthAdjustment * 0.4;
    ...
    yieldAdjustmentFactor = Math.max(
      0.05,
      Math.min(1.0, yieldAdjustmentFactor)
    );
    ...
    const humidityOptimal =
      (plantProfile.minHumidity + plantProfile.maxHumidity) / 2;
    ...
    const soilTypeAdjustment =
      plantProfile.soilTypes && plantProfile.soilTypes.includes(soilType)
        ? 1.0
        : 1.25; // 25% increase for unsuitable soil

    // Seasonal timing cost adjustments
    const seasonalCostFactor = seasonalSuitability.suitable
      ? 1.0
      : seasonalSuitability.score > 0.7
      ? 1.15
      : seasonalSuitability.score > 0.3
      ? 1.35
      : 1.6; // Wrong season dramatically increases costs
    ...
    const environmentalStressFactor =
      tempStressFactor *
      humidityStressFactor *
      pHAdjustmentFactor *
      soilTypeAdjustment *
      seasonalCostFactor *
      weatherCostFactor;
    ...
    console.log('=== COMPREHENSIVE STRESS FACTOR DEBUG ===');
    console.log({
      temperature: {
        min: plantProfile.minTemp,
        max: plantProfile.maxTemp,
        optimal: tempOptimal,
        current: temp,
        deviation: tempDeviation,
        factor: tempStressFactor
      },
      humidity: {
        min: plantProfile.minHumidity,
        max: plantProfile.maxHumidity,
        optimal: humidityOptimal,
        current: humidity,
        deviation: humidityDeviation,
        factor: humidityStressFactor
      },
      soil: {
        pHMin: plantProfile.minSoilPH,
        pHMax: plantProfile.maxSoilPH,
        pHOptimal: pHOptimal,
        pHCurrent: soilPH,
        pHDeviation: pHDeviation,
        pHFactor: pHAdjustmentFactor,
        soilTypes: plantProfile.soilTypes,
        soilTypeCurrent: soilType,
        soilTypeFactor: soilTypeAdjustment
      },
      seasonal: {
        suitable: seasonalSuitability.suitable,
        score: seasonalSuitability.score,
        factor: seasonalCostFactor
      },
      weather: {
        risk: weatherRisk,
        factor: weatherCostFactor
      },
      finalEnvironmentalStressFactor: environmentalStressFactor
    });
    ...
    const laborCost =
      ...
      regionalCostFactor *
      cropFactor *
      environmentalStressFactor;
    const equipmentCost =
      baseEquipmentCost *
      farmSize *
      regionalCostFactor *
      environmentalStressFactor;
    const pestControlCost =
      basePestControlCost *
      farmSize *
      regionalCostFactor *
      diseaseMultiplier *
    ...
    const harvestingCost =
      ...
      regionalCostFactor *
      cropFactor *
      (yieldAdjustmentFactor > 0.5 ? 1.0 : 1.5); // Higher harvesting costs for poor yields
    const miscCost =
      baseMiscCost * farmSize * regionalCostFactor * environmentalStressFactor;
    ...
    const profitMargin =
      adjustedRevenue > 0 ? estimatedProfit / adjustedRevenue : 0;
    const roi = totalCosts > 0 ? (estimatedProfit / totalCosts) * 100 : 0;

    console.log('Final economic calculations:');
    console.log('- idealYield:', idealYield);
    console.log('- adjustedYield:', adjustedYield);
    console.log('- idealRevenue:', idealRevenue);
    console.log('- adjustedRevenue:', adjustedRevenue);
    console.log('- totalCosts:', totalCosts);
    console.log('- estimatedProfit:', estimatedProfit);
    console.log('- profitMargin:', profitMargin);
    console.log('- roi:', roi);
    console.log('=== END ECONOMIC CALCULATIONS DEBUG ===');
    ...
    if (finalHealthIndex < 30 || finalBiomass < 20) {
      result = `Crop failure likely - poor growing conditions. Health: ${finalHealthIndex.toFixed(
        0
      )}%`;
      statusColor = "red";
    } else if (finalHealthIndex < 50 || finalBiomass < 40) {
      result = `Crop will struggle - expect reduced yields. Health: ${finalHealthIndex.toFixed(
        0
      )}%`;
      statusColor = "orange";
    } else if (finalHealthIndex < 70 || finalBiomass < 60) {
      result = `Moderate success expected. Health: ${finalHealthIndex.toFixed(
        0
      )}%`;
      statusColor = "yellow";
    } else if (finalHealthIndex < 85 || finalBiomass < 80) {
      result = `Good growing conditions. Health: ${finalHealthIndex.toFixed(
        0
      )}%`;
    ...
    const debugInfo = {
      csvParsing: {
        seedCostRaw: plant["Seed Cost per Hectare (USD)"],
        expectedYieldRaw: plant["Expected Yield per Hectare (kg)"],
        marketPriceRaw: plant["Market Price per kg (USD)"],
        seedCostConverted: plantProfile.seedCost,
        expectedYieldConverted: plantProfile.expectedYield,
        marketPriceConverted: plantProfile.marketPrice,
      },
      calculations: {
        farmSize: farmSize,
    ...
    res.json({
      ...
      environmental: {
        temperature: {
          ...
          suitable: tempSuitability,
          advice: tempSuitability
            ? "Temperature is ideal"
            : `Adjust to ${plantProfile.tempRange[0]}-${plantProfile.tempRange[1]}°C range`,
        },
        humidity: {
          current: humidity,
          optimal: plantProfile.humidityRange,
          suitable: humiditySuitability,
          advice: humiditySuitability
            ? "Humidity is ideal"
            : `Maintain ${plantProfile.humidityRange[0]}-${plantProfile.humidityRange[1]}% humidity`,
        },
        airQuality: {
          current: aqi,
          optimal: plantProfile.aqiRange,
          suitable: aqiSuitability,
          advice: aqiSuitability
            ? "Air quality is suitable"
      ...
      soil: {
        pH: {
          ...
          adjustment: pHSuitability
            ? "No adjustment needed"
            : soilPH < plantProfile.soilPH[0]
        ...
        type: {
          ...
          improvement: soilTypeSuitability
      ...
      economics: {
        ...
        returns: {
          ...
          adjustedYield: adjustedYield,
          yieldLossPercent:
            ((1 - yieldAdjustmentFactor) * 100).toFixed(1) + "%",
          idealRevenue: idealRevenue,
          adjustedRevenue: adjustedRevenue,
        ...
        breakEven:
          totalCosts > 0
            ? (totalCosts / plantProfile.marketPrice).toFixed(0) + " kg"
            : "N/A",
        marketDemand: plantProfile.marketDemand,
        costPerKg:
          adjustedYield > 0 ? (totalCosts / adjustedYield).toFixed(2) : "N/A",
        yieldReality:
          yieldAdjustmentFactor < 0.5
            ? "Crop likely to fail"
            : yieldAdjustmentFactor < 0.8
            ? "Reduced yields expected"
        ...
        viabilityScore: calculateViabilityScore({
          ...
          environmentalSuitability:
            (tempSuitability + humiditySuitability + aqiSuitability) / 3,
          soilSuitability: (pHSuitability + soilTypeSuitability) / 2,
          seasonalSuitability: seasonalSuitability.suitable,
          riskLevel:
            diseaseRisk === "High" || weatherRisk === "High"
              ? "High"
              : diseaseRisk === "Medium" || weatherRisk === "Medium"
              ? "Medium"
              : "Low",
        }),
      ...
      growth: {
        ...
        growthSummary: `Achieved ${finalBiomass.toFixed(1)}/${K.toFixed(
          1
        )} potential biomass (${finalHealthIndex.toFixed(0)}% health)`,
      },
      ...
      risks: {
        disease: {
          level: diseaseRisk,
          commonDiseases: plantProfile.commonDiseases,
          prevention: generateDiseasePreventionPlan(
            plantProfile.commonDiseases
          ),
        },
        weather: {
          level: weatherRisk,
      ...
      farming: {
        ...
        harvesting: {
          estimatedDate: calculateHarvestDate(
            plantingMonth,
            plantProfile.daysToMaturity
          ),
...
function calculateSoilFactor(soilPH, soilType, plantProfile) {
  ...
  if (plantProfile.minSoilPH !== undefined && soilPH < plantProfile.minSoilPH) {
    ...
    plantProfile.maxSoilPH !== undefined &&
    soilPH > plantProfile.maxSoilPH
  ) {
...
function assessWeatherRisk(temp, humidity, plantProfile) {
  ...
  if (
    plantProfile.minTemp !== undefined &&
    plantProfile.maxTemp !== undefined
  ) {
  ...
  if (
    plantProfile.minHumidity !== undefined &&
    plantProfile.maxHumidity !== undefined
  ) {
    if (
      humidity < plantProfile.minHumidity - 10 ||
      humidity > plantProfile.maxHumidity + 10
    ) {
      return "High";
    }
    if (
...
function generateFarmingAdvice(plantProfile, conditions) {
  ...
  if (conditions.seasonalSuitability === false) {
    ...
    conditions.seasonalMessage &&
    conditions.seasonalMessage.includes("consider")
  ) {
  ...
  if (!conditions.tempSuitability) {
    advice.push(
      `Temperature adjustment needed: optimal range is ${plantProfile.minTemp}-${plantProfile.maxTemp}°C`
    );
  }

  if (!conditions.pHSuitability) {
    advice.push(
      `Soil pH adjustment required: target ${plantProfile.minSoilPH}-${plantProfile.maxSoilPH}`
    );
  }
  ...
  advice.push(
    `Water requirement: ${plantProfile.waterRequirements}mm per season`
  );
...
function generateIrrigationPlan(plantProfile, temp, humidity, farmSize) {
  ...
  const totalCost =
    adjustedWater * costMultiplier + infrastructureCost + maintenanceCost;

  return {
    totalWaterNeeded: Math.round(adjustedWater),
    frequency:
      temp > 25 ? "Daily" : temp > 20 ? "Every 2 days" : "Every 3 days",
    cost: totalCost,
    method: method,
    breakdown: {
    ...
    efficiency: method.includes("Drip")
      ? "High"
      : method.includes("Sprinkler")
      ? "Medium"
      : "Low",
  };
}
...
function generateFertilizationPlan(plantProfile, soilType) {
  ...
  const nitrogenNeeded =
    baseNitrogen * (nutrientMultipliers[plantProfile.nitrogenNeeds] || 1.0);
  const phosphorusNeeded =
    basePhosphorus * (nutrientMultipliers[plantProfile.phosphorusNeeds] || 1.0);
  const potassiumNeeded =
    basePotassium * (nutrientMultipliers[plantProfile.potassiumNeeds] || 1.0);
  ...
  const fertilizerCost =
    finalNitrogen * fertilizerCosts.nitrogen +
    ...
    schedule: [
      {
        ...
        npk: `${Math.round(finalNitrogen * 0.4)}-${Math.round(
          finalPhosphorus * 0.8
        )}-${Math.round(finalPotassium * 0.4)} kg/ha`,
      ...
      },
      {
        ...
        npk: `${Math.round(finalNitrogen * 0.3)}-0-${Math.round(
          finalPotassium * 0.3
        )} kg/ha`,
      ...
      },
      {
        ...
        npk: `${Math.round(finalNitrogen * 0.3)}-${Math.round(
          finalPhosphorus * 0.2
        )}-${Math.round(finalPotassium * 0.3)} kg/ha`,
...
function calculateHarvestDate(plantingMonth, daysToMaturity) {
  ...
  return (
    months[harvestMonth - 1] +
    ` (${monthsToMaturity} months from ${plantingMonth} planting)`
  );
}
...
function assessSeasonalSuitability(plantProfile, plantingMonth) {
  ...
  const hasRelatedMatch = relatedSeasons.some((season) =>
    optimalSeasons.includes(season)
  );
  ...
  return {
    ...
    message: `Poor timing! ${
      plantProfile.name
    } is best planted in ${optimalSeasons.join(" or ")}, not ${plantingMonth}`,
    risk: "High",
  };
}
...
app.post("/api/ai-chat", async (req, res) => {
  try {
    ...
    if (!GEMINI_API_KEY || GEMINI_API_KEY === "YOUR_GEMINI_API_KEY_HERE") {
      console.log(
        "⚠️  Gemini API key not configured, using fallback response system"
      );
      try {
...
app.post("/api/genetic-optimization", (req, res) => {
  try {
    ...
    const { farmConfig, objectives, gaParameters } = req.body;

    // Validate required fields
    if (!farmConfig || !objectives || !gaParameters) {
      throw new Error(
        "Missing required fields: farmConfig, objectives, or gaParameters"
      );
    }
    ...
    const results = simulateGeneticOptimization(
      farmConfig,
      objectives,
      gaParameters
    );
    ...
    res.status(500).json({
      ...
      message:
        error.message || "Unable to complete genetic algorithm optimization.",
    });
  }
});
    ...
    if (mentionedPlant) {
      try {
        ...
        if (plantData && typeof plantData === "object") {
          return `🌱 **Basic Guide for ${
            mentionedPlant.charAt(0).toUpperCase() + mentionedPlant.slice(1)
          }**
...
function simulateGeneticOptimization(farmConfig, objectives, gaParameters) {
  ...
  const { populationSize, generations, crossoverRate, mutationRate } =
    gaParameters;

  console.log(
    `🧬 Starting genetic algorithm optimization for ${farmSize} ha farm in ${climateZone} climate...`
  );

  // Initialize population with realistic constraints
  let population = initializeRealisticPopulation(
    populationSize,
    farmSize,
    budget,
    soilType,
  ...
  for (let generation = 0; generation < generations; generation++) {
    ...
    population.forEach((chromosome) => {
      chromosome.fitness = calculateRealWorldFitness(
        chromosome,
        objectives,
        farmConfig
      );
    ...
    if (generation % 10 === 0) {
      console.log(
        `Generation ${generation}: Best fitness = ${bestFitness.toFixed(4)}`
      );
    }
  }
...
function initializeRealisticPopulation(
  populationSize,
  farmSize,
  budget,
  soilType,
  ...
  for (let i = 0; i < populationSize; i++) {
    ...
    for (let j = 0; j < numCrops; j++) {
      if (j === numCrops - 1) {
        ...
        const allocation =
          Math.random() * (remainingLand * 0.6) + remainingLand * 0.1;
    ...
    for (let j = 0; j < numCrops; j++) {
      ...
      chromosome.pesticidesUse.push(
        (cropData.pesticideCostPerHa * pesticidesVariation) / 100
      );
    }
...
function getClimateAdaptedCrops(climateZone, soilType) {
  ...
  return allCrops.filter((crop) => {
    const cropData = getCropParameters(crop);
    return (
      cropData.climatePreference.includes(climateZone) &&
...
function calculateRealWorldFitness(chromosome, objectives, farmConfig) {
  ...
  const waterEfficiencyScore = calculateWaterEfficiencyScore(
    chromosome,
    farmConfig
  );
  const diseaseResistanceScore = calculateDiseaseResistanceScore(
    chromosome,
    farmConfig
  );
  ...
  if (objectives.optimizeProfitability) {
    fitness += profitabilityScore * 0.3;
    totalWeight += 0.3;
  }
  if (objectives.optimizeSustainability) {
    fitness += sustainabilityScore * 0.2;
    totalWeight += 0.2;
  }
  ...
  if (objectives.optimizeDiseaseResistance) {
    fitness += diseaseResistanceScore * 0.1;
    totalWeight += 0.1;
  }
...
function calculateYieldScore(chromosome, farmConfig) {
  ...
  for (let i = 0; i < chromosome.crops.length; i++) {
    ...
    const waterStressFactor = calculateWaterStressFactor(
      waterUse,
      optimalWater
    );
    const nutrientFactor = calculateNutrientResponseFactor(
      fertilizerUse,
      optimalNitrogen
    );
  ...
  return maxPossibleYield > 0
    ? Math.min(1.0, totalYield / maxPossibleYield)
    : 0;
}
...
function calculateProfitabilityScore(chromosome, farmConfig) {
  ...
  const scaleFactor = getEconomyOfScaleFactor(
    chromosome.landUse.reduce((a, b) => a + b, 0)
  );
...
function calculateSustainabilityScore(chromosome, farmConfig) {
  ...
  for (let i = 0; i < chromosome.crops.length; i++) {
    ...
    const waterEfficiency = Math.max(
      0,
      1 -
        Math.abs(waterUse - cropData.waterRequirement) /
          cropData.waterRequirement
    );
    const fertilizerEfficiency = Math.max(
      0,
      1 -
        Math.abs(fertilizerUse - cropData.nitrogenRequirement) /
          cropData.nitrogenRequirement
    );
    const pesticideScore = Math.max(
      0,
      1 - pesticidesUse / (cropData.pesticideCostPerHa / 100)
    ); // Lower pesticide = higher score
...
function calculateDiseaseResistanceScore(chromosome, farmConfig) {
  ...
  for (let i = 0; i < chromosome.crops.length; i++) {
    ...
    const totalResistanceForCrop = Math.min(
      1,
      baseResistance + pesticideProtection + diversityBonus - climateRisk
    );
...
function getEconomyOfScaleFactor(totalLandArea) {
  ...
  if (totalLandArea > 500) return 0.9; // Medium farm
...
function getSoilHealthImpact(crop) {
  const soilImpact = {
    ...
    Corn: 0.6,
    Rice: 0.5,
    Soybeans: 0.9, // Legumes improve soil
    Tomatoes: 0.7,
    Potatoes: 0.6,
    Cotton: 0.5,
    Sugarcane: 0.4,
...
function calculateGenerationStats(population, farmConfig) {
  const avgFitness =
    population.reduce((sum, chr) => sum + chr.fitness, 0) / population.length;
  const avgSustainability =
    population.reduce(
      (sum, chr) => sum + calculateSustainabilityScore(chr, farmConfig),
      0
    ) / population.length;
  const avgYield =
    population.reduce(
      (sum, chr) => sum + calculateYieldScore(chr, farmConfig),
      0
    ) / population.length;
  ...
  for (let i = 0; i < chromosome.landUse.length; i++) {
    ...
    if (Math.random() < 0.2) {
      ...
      chromosome.fertilizerAllocation[i] = Math.max(
        ...
        Math.min(
          cropData.nitrogenRequirement * 1.8,
...
function calculateComprehensiveMetrics(bestSolution, farmConfig) {
  ...
  const waterEfficiency = calculateWaterEfficiencyScore(
    bestSolution,
    farmConfig
  );
  const diseaseResistance = calculateDiseaseResistanceScore(
    bestSolution,
    farmConfig
  );
...
function generateSmartRecommendations(bestSolution, metrics, farmConfig) {
  ...
  if (parseFloat(metrics.totalYield) < farmConfig.farmSize * 5) {
    recommendations.push(
      "Consider higher-yielding crop varieties or improved cultivation techniques to increase productivity"
    );
  }
  ...
  if (parseFloat(metrics.waterEfficiency) < 0.7) {
    recommendations.push(
      "Install drip irrigation or implement water-saving techniques to improve water use efficiency"
    );
  }
  ...
  if (parseFloat(metrics.sustainability) < 0.7) {
    recommendations.push(
      "Adopt organic farming practices and reduce chemical inputs for better environmental sustainability"
    );
  }
  ...
  if (parseFloat(metrics.profitability) < 0.6) {
    recommendations.push(
      "Focus on high-value crops and optimize input costs to improve farm profitability"
    );
  }
  ...
  if (parseFloat(metrics.diseaseResistance) < 0.6) {
    recommendations.push(
      "Implement integrated pest management and consider disease-resistant crop varieties"
    );
  }
  ...
  if (Object.keys(cropCounts).length < 3) {
    recommendations.push(
      "Increase crop diversity to reduce disease risk and improve soil health through rotation"
    );
  }
  ...
  if (farmConfig.climateZone === "Arid") {
    recommendations.push(
      "Focus on drought-tolerant crops and water conservation techniques for arid climate conditions"
    );
  } else if (farmConfig.climateZone === "Tropical") {
    recommendations.push(
      "Implement pest management strategies suitable for high humidity and temperature conditions"
    );
  }
...
function calculateComprehensiveViability({
  ...
  if (budgetRatio >= 1.2) {
    ...
    budgetStatus =
      "Tight budget - need $" + (totalCosts - budget).toFixed(0) + " more";
  } else {
    budgetStatus =
      "Insufficient budget - need $" +
      (totalCosts - budget).toFixed(0) +
      " more";
  }
...
function identifyRiskFactors({
  ...
  if (tempDeviation > 5) {
    risks.push({
      ...
      description: `${tempDeviation.toFixed(
        1
      )}°C deviation from optimal ${tempOptimal.toFixed(1)}°C`,
  ...
  if (humidityDeviation > 15) {
    risks.push({
      ...
      description: `${humidityDeviation.toFixed(
        1
      )}% deviation from optimal ${humidityOptimal.toFixed(1)}%`,
  ...
  if (budgetRatio < 1.0) {
    risks.push({
      ...
      description: `Need $${(totalCosts - budget).toFixed(0)} more (${(
        (1 - budgetRatio) *
        100
      ).toFixed(1)}% short)`,
...
// NEW: Genetic Algorithm-Based Agricultural Optimization Endpoint
// Based on Lecture 10 - Genetic Algorithms for Ecological Optimization
app.post("/api/optimize-agriculture-ga", async (req, res) => {
  try {
    const { city, soilType, soilPH, farmSize, budget, plantingMonth } =
      req.body;

    console.log(`🧬 Starting GA-based agricultural optimization for ${city}`);

    // Mock coordinates and weather data for demonstration
    const coordinates = { lat: 40.7128, lon: -74.006 }; // Default to NYC coords
    const weatherData = {
      temp: 20 + Math.random() * 10, // 20-30°C
      humidity: 50 + Math.random() * 30, // 50-80%
      aqi: 30 + Math.random() * 40, // 30-70 AQI
    };

    // Determine climate zone based on coordinates
    const climateZone =
      Math.abs(coordinates.lat) <= 23.5
        ? "Tropical"
        : Math.abs(coordinates.lat) <= 35
        ? "Subtropical"
        : Math.abs(coordinates.lat) <= 50
        ? "Temperate"
        : "Continental";

    // Configure genetic algorithm parameters (following Lec 10 recommendations)
    const gaParameters = {
      populationSize: 100, // 50-200 recommended for ecological problems
      generations: 150, // 100-1000 depending on complexity
      crossoverRate: 0.8, // 0.7-0.9 for good exploration/exploitation balance
      mutationRate: 0.05, // 0.01-0.1 for diversity maintenance
    };
    ...
    // Multi-objective optimization (following Lec 10 principles)
    const objectives = {
      yield: 0.4, // 40% weight on yield maximization
      profit: 0.3, // 30% weight on profit maximization
      sustainability: 0.2, // 20% weight on sustainability
      waterEfficiency: 0.1, // 10% weight on water efficiency
    };

    // Run genetic algorithm optimization
    const optimizationResult = simulateGeneticOptimization(
      farmConfig,
      objectives,
      gaParameters
    );
    ...
    const viabilityScore = Math.round(
      optimizationResult.bestSolution.fitness * 100
    );

    // Response following the structure expected by the map system
    const response = {
      city: city,
      ...
      // Optimized Results
      optimizedSolution: {
        crops: optimizationResult.bestSolution.crops,
        landAllocation: optimizationResult.bestSolution.landUse,
        expectedYield: optimizationResult.metrics.totalYield,
        expectedProfit: optimizationResult.metrics.totalProfit,
        fitnessScore: optimizationResult.bestSolution.fitness,
        sustainabilityScore: optimizationResult.metrics.sustainabilityScore,
      },
      ...
      environmental: {
        temperature: {
          ...
          suitability:
            weatherData.temp >= 15 && weatherData.temp <= 30
              ...
              : "Challenging",
          status:
            weatherData.temp >= 15 && weatherData.temp <= 30 ? "✅" : "⚠️",
        },
        humidity: {
        ...
        airQuality: {
          ...
          suitability: weatherData.aqi <= 100 ? "Good" : "Poor",
          status: weatherData.aqi <= 100 ? "✅" : "❌",
        },
      },
      ...
      soil: {
        ...
        pH: soilPH,
        suitability:
          soilPH >= 6.0 && soilPH <= 7.5 ? "Optimal" : "Needs adjustment",
      },

      // Economic Analysis
      economics: {
        totalInvestment: optimizationResult.metrics.totalCosts,
        expectedRevenue: optimizationResult.metrics.totalRevenue,
        netProfit: optimizationResult.metrics.totalProfit,
        roi: optimizationResult.metrics.roi,
        budgetUtilization:
          (optimizationResult.metrics.totalCosts / budget) * 100,
      },
      ...
      optimization: {
        ...
        convergenceData: optimizationResult.convergenceData,
        populationDiversity: optimizationResult.metrics.diversity || 0.8,
      },

      // Recommendations based on GA results
      recommendations: {
        primary: optimizationResult.recommendations || [
          `Optimal crop mix: ${optimizationResult.bestSolution.crops
            .map((c) => c.name)
            .join(", ")}`,
          `Expected total yield: ${optimizationResult.metrics.totalYield.toFixed(
            1
          )} tonnes`,
          `Projected profit: $${optimizationResult.metrics.totalProfit.toFixed(
            0
          )}`,
        ],
        advantages: [
          "Multi-objective optimization considers yield, profit, and sustainability",
          "Climate-adapted crop selection based on genetic algorithm evolution",
          "Resource allocation optimization through iterative improvement",
          "Risk-aware recommendations based on population diversity",
        ],
      },
    };

    console.log(
      `✅ GA optimization complete for ${city}: Score ${viabilityScore}/100`
    );
    res.json(response);
  } catch (error) {
    console.error("GA optimization error:", error);
    res.status(500).json({
      error: "Agricultural optimization failed",
      details: error.message,
    });
  }
});

// Alternative endpoint using Kriging for spatial interpolation (Lec 6-7)
app.post("/api/spatial-agriculture-kriging", async (req, res) => {
  try {
    const { targetCity, knownCities, soilType, farmSize, budget } = req.body;

    console.log(
      `🗺️ Starting Kriging-based spatial agricultural analysis for ${targetCity}`
    );

    // Mock implementation of Kriging principles from Lecture 6-7
    // In a real implementation, this would use actual spatial interpolation
    const krigingResult = {
      predictedSuitability: 0.75 + Math.random() * 0.25, // 75-100% suitability
      predictionUncertainty: 0.05 + Math.random() * 0.15, // 5-20% uncertainty
      spatialCorrelation: 0.8, // Strong spatial correlation
      nearestNeighbors: knownCities?.slice(0, 3) || [
        "Default City 1",
        "Default City 2",
        "Default City 3",
      ],
    ...
    const response = {
      ...
      algorithm: "Kriging Spatial Interpolation",
      spatial: {
        predictedSuitability:
          (krigingResult.predictedSuitability * 100).toFixed(1) + "%",
        uncertainty:
          (krigingResult.predictionUncertainty * 100).toFixed(1) + "%",
        confidence:
          ((1 - krigingResult.predictionUncertainty) * 100).toFixed(1) + "%",
        spatialCorrelation: krigingResult.spatialCorrelation,
      },
      recommendations: [
        `Predicted agricultural suitability: ${(
          krigingResult.predictedSuitability * 100
        ).toFixed(1)}%`,
        `Prediction uncertainty: ±${(
          krigingResult.predictionUncertainty * 100
        ).toFixed(1)}%`,
        `Based on spatial correlation with nearby cities`,
        `Recommend additional sampling if uncertainty > 15%`,
      ],
      methodology:
        "Geostatistical interpolation using variogram modeling and spatial autocorrelation",
    };

    res.json(response);
  } catch (error) {
    console.error("Kriging analysis error:", error);
    res.status(500).json({
